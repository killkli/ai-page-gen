...
| #   | 功能               | 工時 | 影響 | 實作說明                                         | 狀態    |
| --- | ------------------ | ---- | ---- | ------------------------------------------------ | ------- |
| 1   | **記憶卡鍵盤支援** | 2h   | 高   | 在 `MemoryCardGameQuizItem.tsx` 加入 `onKeyDown` | ✅ 完成 |
| 2   | **列印樣式**       | 2h   | 中   | 在 `index.html` 加入 `@media print` 規則         | ✅ 完成 |
| 3   | **骨架載入器**     | 3h   | 中   | 建立 `SkeletonLoader.tsx` 元件                   | ✅ 完成 |
| 4   | **測驗嘗試計數**   | 1h   | 低   | 顯示嘗試次數於記憶卡遊戲                         | ✅ 完成 |
| 5   | **難度偏好保存**   | 30m  | 低   | 將上次難度存入 localStorage                      | ✅ 完成 |
| 6   | **導航確認**       | 1h   | 中   | 未完成測驗時 `beforeunload` 警告                 | ✅ 完成 |
| 7   | **可操作錯誤訊息** | 2h   | 中   | 建立 `ErrorDisplay.tsx` 元件含重試按鈕           | ✅ 完成 |
| 8   | **焦點管理**       | 2h   | 高   | 內容載入完成後自動聚焦第一區塊                   | ✅ 完成 |

**總計**: ~14 小時 ✅ **全部完成 (2025-01-07)**
