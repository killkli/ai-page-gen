...
Please respond in JSON format.`);const o={model:n,messages:[{role:"user",content:r}],stream:e.options?.stream??t.settings.stream??!1,max_tokens:e.options?.maxTokens??t.settings.max_tokens,temperature:e.options?.temperature??t.settings.temperature,top_p:t.settings.top_p,frequency_penalty:t.settings.frequency_penalty,presence_penalty:t.settings.presence_penalty};return t.settings.provider&&(o.provider=t.settings.provider),Object.keys(o).forEach(c=>{o[c]===void 0&&delete o[c]}),o}async executeApiCall(e){const t=await fetch(`${this.baseUrl}/chat/completions`,{method:"POST",headers:{Authorization:`Bearer ${this.config.apiKey}`,"Content-Type":"application/json","HTTP-Referer":window.location.origin,"X-Title":"AI Learning Page Generator"},body:JSON.stringify(e)});if(!t.ok){const n=await t.json().catch(()=>({}));throw new Error(n.error?.message||`HTTP ${t.status}: ${t.statusText}`)}return await t.json()}parseApiResponse(e){if(!e.choices||e.choices.length===0)throw new Error("OpenRouter 回應格式錯誤：缺少 choices");const t=e.choices[0];if(!t.message||!t.message.content)throw new Error("OpenRouter 回應格式錯誤：缺少訊息內容");const n=t.message.content.trim();console.log("OpenRouter 回應內容長度:",n.length),console.log("OpenRouter 回應內容前100字:",n.substring(0,100)),console.log("OpenRouter 回應內容後100字:",n.substring(Math.max(0,n.length-100)));const r=n.replace(/```json\n/g,"").replace(/\n```/g,"").replace(/```\n/g,"").replace(/```/g,"").trim();if(console.log("OpenRouter 清理後內容前100字:",r.substring(0,100)),r.startsWith("{")||r.startsWith("["))try{const s=JSON.parse(r);return console.log("OpenRouter JSON 解析成功:",typeof s,Object.keys(s||{})),s}catch(s){return console.error("OpenRouter 回應 JSON 解析失敗:",String(s)),console.log("JSON 解析失敗的內容:",r),r}return console.log("OpenRouter 回應不是 JSON 格式，返回清理後文字"),r}extractUsageInfo(e,...t){const n=t[0];if(n&&n.usage)return{promptTokens:n.usage.prompt_tokens,completionTokens:n.usage.completion_tokens,totalTokens:n.usage.total_tokens}}formatResponse(e,t,...n){try{const r=n[0];return{success:!0,data:e,provider:this.config.name,model:t,usage:r?.usage?{promptTokens:r.usage.prompt_tokens,completionTokens:r.usage.completion_tokens,totalTokens:r.usage.total_tokens}:void 0}}catch(r){return this.handleError(r,"格式化回應數據")}}getEffectiveModel(e){const t=this.config;return e.options?.model||t.model}handleError(e,t){let n="無法產生內容。";if(e instanceof Error&&(n+=` 詳細資料： ${e.message}`),e&&typeof e=="object"&&"message"in e){const r=e;r.message.includes("Unauthorized")||r.message.includes("401")?n="OpenRouter API 金鑰無效。請檢查您的設定。":r.message.includes("rate limit")||r.message.includes("429")?n="已超出 API 速率限制。請稍後再試。":r.message.includes("quota")||r.message.includes("insufficient")?n="已超出 API 配額或餘額不足。請檢查您的 OpenRouter 帳戶。":r.message.includes("model")||r.message.includes("not found")?n="所選模型不可用。請嘗試其他模型。":(r.message.includes("timeout")||r.message.includes("network"))&&(n="網路連接超時。請檢查網路連接後重試。")}return{success:!1,error:n,provider:this.config.name,model:this.getDefaultModel()}}async getAvailableModels(){try{const e=await fetch(`${this.baseUrl}/models`,{headers:{Authorization:`Bearer ${this.config.apiKey}`,"Content-Type":"application/json"}});if(!e.ok)throw new Error(`HTTP ${e.status}: ${e.statusText}`);return(await e.json()).data||[]}catch(e){return console.error("獲取 OpenRouter 模型清單失敗:",e),[]}}}class y{constructor(e){this.providers=new Map,this.usageStats=new Map,this.config=e,this.initializeProviders(),this.loadUsageStats()}static getInstance(e){return y.instance||(e||(e={providers:[],fallbackEnabled:!0,timeout:3e4,retryAttempts:3}),y.instance=new y(e)),y.instance}static reloadWithConfig(e){return y.instance=new y(e),y.instance}initializeProviders(){this.config.providers.forEach(e=>{try{const t=this.createProvider(e);this.providers.set(e.id,t)}catch(t){console.error(`初始化 Provider ${e.id} 失敗:`,t)}})}createProvider(e){switch(e.type){case w.GEMINI:return new Y(e);case w.OPENROUTER:return new X(e)}}async addProvider(e){try{const t=this.createProvider(e),n=await this.testProvider(e.id,t);if(n.status===b.ERROR)throw new Error(`Provider 測試失敗: ${n.error}`);this.providers.set(e.id,t),this.config.providers.push(e),this.saveConfig(),console.log(`Provider ${e.id} 已成功添加`)}catch(t){throw console.error(`添加 Provider ${e.id} 失敗:`,t),t}}removeProvider(e){this.providers.delete(e),this.config.providers=this.config.providers.filter(t=>t.id!==e),this.usageStats.delete(e),this.saveConfig(),console.log(`Provider ${e} 已移除`)}async updateProvider(e){try{const t=this.createProvider(e),n=await this.testProvider(e.id,t);if(n.status===b.ERROR)throw new Error(`Provider 測試失敗: ${n.error}`);this.providers.set(e.id,t);const r=this.config.providers.findIndex(s=>s.id===e.id);r>=0&&(this.config.providers[r]=e),this.saveConfig(),console.log(`Provider ${e.id} 已更新`)}catch(t){throw console.error(`更新 Provider ${e.id} 失敗:`,t),t}}async testProvider(e,t){const n=Date.now(),r=t||this.providers.get(e);if(!r)return{providerId:e,status:b.ERROR,error:"Provider 不存在",testedAt:new Date().toISOString()};try{const s={prompt:'測試連接：請回應 "OK"',options:{maxTokens:500,responseFormat:"text"}},a=await r.generateContent(s),o=Date.now()-n;return{providerId:e,status:a.success?b.ACTIVE:b.ERROR,responseTime:o,error:a.error,testedAt:new Date().toISOString()}}catch(s){return{providerId:e,status:b.ERROR,error:s instanceof Error?s.message:"未知錯誤",testedAt:new Date().toISOString()}}}async testAllProviders(){const e=[];for(const[t,n]of this.providers){const r=await this.testProvider(t,n);e.push(r)}return e}async generateContent(e,t=S.DEFAULT){const n=await this.selectProvider(t);if(!n)throw new B({type:v.UNKNOWN_ERROR,message:"沒有可用的 Provider",provider:"none",retryable:!1});const r=Date.now();try{const s=await n.provider.generateContent(e),a=Date.now()-r;if(!s.success)throw new B({type:this.mapErrorType(s.error||""),message:s.error||"未知錯誤",provider:n.id,retryable:!0});return this.updateUsageStats(n.id,!0,a,s.usage?.totalTokens||0),{content:s.data,usage:s.usage,metadata:{model:s.model,provider:s.provider,responseTime:a,finishReason:"stop"}}}catch(s){if(this.updateUsageStats(n.id,!1,Date.now()-r,0),this.config.fallbackEnabled&&t!==S.FALLBACK)return console.warn(`Provider ${n.id} 失敗，嘗試 fallback...`),this.generateContent(e,S.FALLBACK);throw s}}async selectProvider(e){const t=Array.from(this.providers.entries()).filter(([n,r])=>r.isEnabled()).map(([n,r])=>({id:n,provider:r}));if(console.log("ProviderManager: 可用的 Provider:",t.map(n=>({id:n.id,type:n.provider.config.type}))),t.length===0)return console.log("ProviderManager: 沒有可用的 Provider"),null;switch(e){case S.DEFAULT:{const n=this.config.defaultProviderId;if(console.log("ProviderManager: 預設 Provider ID:",n),n&&this.providers.has(n)){const r={id:n,provider:this.providers.get(n)};return console.log("ProviderManager: 選擇預設 Provider:",{id:r.id,type:r.provider.config.type}),r}return console.log("ProviderManager: 預設 Provider 不可用，使用第一個可用的 Provider:",t[0].id),t[0]}case S.FASTEST:return this.selectFastestProvider(t);case S.FALLBACK:return this.selectFallbackProvider(t);case S.LOAD_BALANCE:return this.selectLoadBalanceProvider(t);default:return t[0]}}selectFastestProvider(e){let t=e[0],n=this.getAverageResponseTime(t.id);for(const r of e.slice(1)){const s=this.getAverageResponseTime(r.id);s<n&&(t=r,n=s)}return t}selectFallbackProvider(e){for(const t of this.config.providers){const n=e.find(r=>r.id===t.id);if(n)return n}return e[0]}selectLoadBalanceProvider(e){const t=Date.now(),n=Math.floor(t/1e3)%e.length;return e[n]}getAverageResponseTime(e){return this.usageStats.get(e)?.averageResponseTime||1/0}updateUsageStats(e,t,n,r){const s=this.usageStats.get(e)||{providerId:e,totalRequests:0,successfulRequests:0,failedRequests:0,averageResponseTime:0,totalTokensUsed:0,lastUsed:new Date().toISOString()};s.totalRequests++,t?s.successfulRequests++:s.failedRequests++;const a=s.averageResponseTime*(s.totalRequests-1)+n;s.averageResponseTime=a/s.totalRequests,s.totalTokensUsed+=r,s.lastUsed=new Date().toISOString(),this.usageStats.set(e,s),this.saveUsageStats()}mapErrorType(e){const t=e.toLowerCase();return t.includes("api key")||t.includes("invalid")?v.API_KEY_INVALID:t.includes("quota")||t.includes("exhausted")?v.QUOTA_EXCEEDED:t.includes("rate limit")?v.RATE_LIMIT_EXCEEDED:t.includes("model")||t.includes("not found")?v.MODEL_NOT_AVAILABLE:t.includes("network")||t.includes("connection")?v.NETWORK_ERROR:t.includes("json")||t.includes("parsing")?v.PARSING_ERROR:v.UNKNOWN_ERROR}getProviders(){return[...this.config.providers]}getProvider(e){return this.config.providers.find(t=>t.id===e)}getUsageStats(){return Array.from(this.usageStats.values())}setDefaultProvider(e){if(!this.providers.has(e))throw new Error(`Provider ${e} 不存在`);this.config.defaultProviderId=e,this.saveConfig()}saveConfig(){try{localStorage.setItem("provider_manager_config",JSON.stringify(this.config))}catch(e){console.error("保存 Provider 配置失敗:",e)}}loadUsageStats(){try{const e=localStorage.getItem("provider_usage_stats");if(e){const t=JSON.parse(e);this.usageStats=new Map(Object.entries(t))}}catch(e){console.error("載入使用統計失敗:",e)}}saveUsageStats(){try{const e=Object.fromEntries(this.usageStats);localStorage.setItem("provider_usage_stats",JSON.stringify(e))}catch(e){console.error("保存使用統計失敗:",e)}}clearUsageStats(){this.usageStats.clear(),localStorage.removeItem("provider_usage_stats")}exportConfig(){const e={version:"1.0.0",exportedAt:new Date().toISOString(),configs:this.config.providers.map(t=>({...t,apiKey:""})),metadata:{appVersion:"1.0.0",description:"AI Provider 配置匯出"}};return JSON.stringify(e,null,2)}async importConfig(e){try{const t=JSON.parse(e);if(!t.configs||!Array.isArray(t.configs))throw new Error("無效的配置格式");for(const n of t.configs){if(!n.apiKey){console.warn(`跳過 ${n.name}：缺少 API Key`);continue}await this.addProvider({...n,id:`${n.type}_${Date.now()}`,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()})}}catch(t){throw console.error("匯入配置失敗:",t),t}}}const Z={BASE_URL:"/ai-page-gen/",DEV:!1,MODE:"production",PROD:!0,SSR:!1,VITE_APP_NAME:"AI English Teaching Generator",VITE_ENABLE_BRANCH_RECOMMENDATION:"false",VITE_ENABLE_MATH_RENDER:"false",VITE_ENABLE_SPEECH:"true",VITE_JSONBIN_API_KEY:"$2a$10$0PieG5Wm.V.6svtN4PbaTuVxU4LY0nL2Vaz4N3g2aYlb0cL1NG4Qa"};var ee={};const G="https://api.jsonbin.io/v3",J=import.meta&&Z&&"$2a$10$0PieG5Wm.V.6svtN4PbaTuVxU4LY0nL2Vaz4N3g2aYlb0cL1NG4Qa"||ee.VITE_JSONBIN_API_KEY||"";class A{constructor(){this.manager=null,this.initializeManager()}static getInstance(){return A.instance||(A.instance=new A),A.instance}async initializeManager(){try{const e=this.loadManagerConfig();this.manager=y.reloadWithConfig(e)}catch(e){console.error("初始化 Provider Manager 失敗:",e);const t={providers:[],fallbackEnabled:!0,timeout:3e4,retryAttempts:3};this.manager=y.reloadWithConfig(t)}}loadManagerConfig(){const e=localStorage.getItem("provider_manager_config");if(e)try{return JSON.parse(e)}catch(r){console.error("解析 Provider 配置失敗:",r)}const t=localStorage.getItem("gemini_api_key"),n=[];if(t){const r={id:"gemini_migrated",name:"Gemini (已遷移)",type:w.GEMINI,enabled:!0,apiKey:t,model:"gemini-2.5-flash",settings:{responseMimeType:"application/json",temperature:.7,maxOutputTokens:8192},createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};n.push(r)}return{providers:n,defaultProviderId:n.length>0?n[0].id:void 0,fallbackEnabled:!0,timeout:3e4,retryAttempts:3}}async getManager(){return this.manager||await this.initializeManager(),this.manager}async getProviders(){return(await this.getManager()).getProviders()}async addProvider(e){await(await this.getManager()).addProvider(e),await this.initializeManager()}async updateProvider(e){await(await this.getManager()).updateProvider(e),await this.initializeManager()}async removeProvider(e){(await this.getManager()).removeProvider(e),await this.initializeManager()}async testProvider(e){return await(await this.getManager()).testProvider(e)}async testAllProviders(){return await(await this.getManager()).testAllProviders()}async setDefaultProvider(e){(await this.getManager()).setDefaultProvider(e)}async getUsageStats(){return(await this.getManager()).getUsageStats()}async generateContent(e,t){return await(await this.getManager()).generateContent(e,t)}async exportProviderConfig(){return(await this.getManager()).exportConfig()}async importProviderConfig(e){await(await this.getManager()).importConfig(e)}async shareProviderConfig(e,t,n,r=!1){try{const s={id:`share_${Date.now()}`,name:t,description:n,configs:e.map(c=>({...c,apiKey:"",id:"",createdAt:"",updatedAt:""})),isPublic:r,createdAt:new Date().toISOString(),tags:this.extractTagsFromConfigs(e)},a=await fetch(`${G}/b`,{method:"POST",headers:{"Content-Type":"application/json","X-Master-Key":J},body:JSON.stringify(s)});if(!a.ok)throw new Error(`分享失敗: ${a.statusText}`);return(await a.json()).metadata.id}catch(s){throw console.error("分享 Provider 配置失敗:",s),s}}async loadSharedProviderConfig(e){try{const t=await fetch(`${G}/b/${e}/latest`,{headers:{"X-Master-Key":J}});if(!t.ok)throw new Error(`載入失敗: ${t.statusText}`);return(await t.json()).record}catch(t){throw console.error("載入分享的 Provider 配置失敗:",t),t}}async createProvidersFromShared(e,t){const n=[];for(const r of e.configs){const s=t[r.type];if(!s){console.warn(`跳過 ${r.name}：缺少 ${r.type} API Key`);continue}const a={...r,id:`${r.type}_${Date.now()}_${Math.random().toString(36).substr(2,9)}`,apiKey:s,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),name:`${r.name} (已匯入)`};await this.addProvider(a),n.push(a)}return n}extractTagsFromConfigs(e){const t=new Set;return e.forEach(n=>{if(t.add(n.type),n.type===w.GEMINI)t.add("google"),t.add("multimodal");else if(n.type===w.OPENROUTER){t.add("unified-api");const r=n.model;r.includes("openai")&&t.add("openai"),r.includes("anthropic")&&t.add("anthropic"),r.includes("google")&&t.add("google"),r.includes("meta")&&t.add("meta")}}),Array.from(t)}async clearAllData(){localStorage.removeItem("provider_manager_config"),localStorage.removeItem("provider_usage_stats"),this.manager=null,await this.initializeManager()}async hasConfiguredProviders(){const e=await this.getProviders();return e.length>0&&e.some(t=>t.enabled&&t.apiKey)}async getFirstAvailableProvider(){return(await this.getProviders()).find(t=>t.enabled&&t.apiKey)||null}async migrateLegacyApiKey(){const e=localStorage.getItem("gemini_api_key");if(!e)return;if(!(await this.getProviders()).some(r=>r.type===w.GEMINI)){const r={id:"gemini_migrated",name:"Gemini (已遷移)",type:w.GEMINI,enabled:!0,apiKey:e,model:"gemini-2.5-flash",settings:{responseMimeType:"application/json",temperature:.7,maxOutputTokens:8192},createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};try{await this.addProvider(r),await this.setDefaultProvider(r.id),console.log("已成功遷移 Gemini API Key 到新的 Provider 系統")}catch(s){console.error("遷移 Gemini API Key 失敗:",s)}}}async getLegacyApiKey(){return(await this.getFirstAvailableProvider())?.apiKey||null}}const O=A.getInstance(),m=async(i,e="provider-system-call")=>{try{!await O.hasConfiguredProviders()&&e!=="provider-system-call"&&await O.migrateLegacyApiKey();const n={prompt:i,options:{responseFormat:"json",maxTokens:8192,temperature:.7}};console.log("Provider 系統: 生成內容中...");const r=await O.generateContent(n,S.DEFAULT);console.log("Provider 系統回應:",{provider:r.metadata?.provider,model:r.metadata?.model});let s=r.content;if(s&&typeof s=="object"&&s.choices)try{s=s.choices[0].message.content,console.log("✓ 檢測到 OpenRouter 格式，提取內容")}catch(a){console.warn("⚠ OpenRouter 格式提取失敗:",a)}if(typeof s=="string")try{let a=s.replace(/```json\n/g,"").replace(/\n```/g,"").replace(/```\n/g,"").replace(/```/g,"").trim();if(!a.startsWith("{")&&!a.startsWith("[")){const o=Math.min(a.indexOf("{")!==-1?a.indexOf("{"):1/0,a.indexOf("[")!==-1?a.indexOf("["):1/0);if(o!==1/0){const l=a.charAt(o)==="{"?a.lastIndexOf("}"):a.lastIndexOf("]");l>o&&(a=a.substring(o,l+1))}}s=JSON.parse(a),console.log("✓ JSON 解析成功 (Provider 系統)")}catch(a){console.warn("⚠ JSON 解析失敗，返回原始內容:",a),s=r.content}return s}catch(t){if(console.error("Provider 系統錯誤:",t),e&&e.startsWith("AIza")){console.log("回退到原始 Gemini 服務");const{callGemini:n}=await R(async()=>{const{callGemini:r}=await import("./gemini-service-Byb1d6yD.js").then(s=>s.r);return{callGemini:r}},__vite__mapDeps([0,1,2,3]));return await n(i,e)}else throw t}},T=async(i,e)=>{const t=`
  ...
  `;return await m(t,e)},te=async(i,e,t)=>{const n=/english|英語|英文|grammar|vocabulary|pronunciation|speaking|listening|reading|writing/i.test(i),r=`
    Based on the following learning objectives: ${JSON.stringify(t)}
    Please break down the topic "${i}" into a detailed curriculum structure.
  ...
  `;return await m(r,e)},ne=async(i,e,t)=>{const n=`
    Based on the following learning objectives: ${JSON.stringify(t)}
    Generate at least 3 (but more is better if appropriate) comprehensive analysis of common misconceptions or difficulties students may have with "${i}".
    ...
    Do NOT include any explanation or extra text. Only output the JSON array.
  `;return await m(n,e)},re=async(i,e,t)=>{const n=`
    Based on the following learning objectives: ${JSON.stringify(t)}
    ...
    Do NOT include any explanation or extra text. Only output the JSON array.
  `;return await m(n,e)},ie=async(i,e,t)=>{const n=`
    Based on the following learning objectives: ${JSON.stringify(t)}
    Please generate quiz content for "${i}" in the following JSON structure (no explanation, no extra text):
    {
      "easy": {
        "trueFalse": [
  ...
  `;return await m(n,e)},se=async(i,e,t)=>{const n=`
    Based on the following learning objectives: ${JSON.stringify(t)}
    ...
    Do NOT include any explanation or extra text. Only output the JSON array.
  `;return await m(n,e)},Ge=async(i,e,t)=>{const n=`
  ...
  `;return await t(n,e)},Je=async(i,e,t)=>{const n=`
  ...
  `;return await t(n,e)},Fe=async(i,e,t)=>{const n=`
    ...
    Use Traditional Chinese. Do NOT include any explanation or extra text. Only output the JSON object.
  `;return await t(n,e)},ae=async(i,e,t)=>{const n=`
    ...
    Do NOT include any explanation or extra text. Only output the JSON array.
  `;return await m(n,t)},oe=async(i,e,t,n)=>{const r=/english|英語|英文|grammar|vocabulary|pronunciation|speaking|listening|reading|writing/i.test(i),s=`
    Based on the following learning objectives: ${JSON.stringify(n)}
    Please break down the topic "${i}" into at least 3 (but more is better if appropriate) micro-units appropriate for "${e.name}" level learners (${e.description}). For each, provide a sub-topic, a brief explanation, and a concrete teaching example.
    ...
    Do NOT include any explanation or extra text. Only output the JSON array.
  `;return await m(s,t)},ce=async(i,e,t,n)=>{const r=`
    Based on the following learning objectives: ${JSON.stringify(n)}
    Generate at least 3 (but more is better if appropriate) comprehensive analysis of common misconceptions or difficulties that "${e.name}" level learners (${e.description}) may have with "${i}".
    ...
    Do NOT include any explanation or extra text. Only output the JSON array.
  `;return await m(r,t)},le=async(i,e,t,n)=>{const r=`
    Based on the following learning objectives: ${JSON.stringify(n)}
    ...
    Do NOT include any explanation or extra text. Only output the JSON array.
  `;return await m(r,t)},de=async(i,e,t,n)=>{const r=`
  ...
  `;return await m(r,t)},ue=async(i,e,t,n)=>{const r=`
    Based on the following learning objectives: ${JSON.stringify(n)}
    ...
    Do NOT include any explanation or extra text. Only output the JSON array.
  `;return await m(r,t)},pe=async(i,e,t,n)=>{const r=`
    ...
    Do NOT include any explanation or extra text. Only output the JSON array.
  `;return await m(r,n)},ge=async(i,e,t,n,r)=>{const s=/english|英語|英文|grammar|vocabulary|pronunciation|speaking|listening|reading|writing/i.test(i),a=`
    Based on the following learning objectives: ${JSON.stringify(r)}
    Please break down the topic "${i}" into at least 3 (but more is better if appropriate) micro-units appropriate for "${e.name}" level learners (${e.description}) with English vocabulary level "${t.name}" (${t.wordCount} words: ${t.description}).
    ...
    Do NOT include any explanation or extra text. Only output the JSON array.
  `;return await m(a,n)},me=async(i,e,t,n,r)=>{const s=`
    Based on the following learning objectives: ${JSON.stringify(r)}
    Generate at least 3 (but more is better if appropriate) comprehensive analysis of common misconceptions or difficulties that "${e.name}" level learners (${e.description}) with English vocabulary level "${t.name}" (${t.wordCount} words: ${t.description}) may have with "${i}".
    ...
    Do NOT include any explanation or extra text. Only output the JSON array.
  `;return await m(s,n)},he=async(i,e,t,n,r)=>{const s=`
    Based on the following learning objectives: ${JSON.stringify(r)}
    ...
    Do NOT include any explanation or extra text. Only output the JSON array.
  `;return await m(s,n)},fe=async(i,e,t,n,r)=>{const s=`
    Based on the following learning objectives: ${JSON.stringify(r)}
  ...
  `;return await m(s,n)},ye=async(i,e,t,n,r)=>{const s=`
    Based on the following learning objectives: ${JSON.stringify(r)}
    ...
    Do NOT include any explanation or extra text. Only output the JSON array.
  `;return await m(s,n)},we=async(i,e,t,n,r,s)=>{const o=(g=>{const u={...g};for(const f of["easy","normal","hard"]){u[f]||(u[f]={});const h=u[f];h.trueFalse=Math.min(Math.max(h.trueFalse||0,0),10),h.multipleChoice=Math.min(Math.max(h.multipleChoice||0,0),10),h.fillInTheBlanks=Math.min(Math.max(h.fillInTheBlanks||0,0),10),h.sentenceScramble=Math.min(Math.max(h.sentenceScramble||0,0),10),h.memoryCardGame=Math.min(Math.max(h.memoryCardGame||0,0),2)}return u})(n),c=async g=>{const u=o[g];let f="";r&&(f=`適合「${r.name}」程度學習者 (${r.description}) 的`);let h="";s&&(h=`
      CRITICAL VOCABULARY CONSTRAINTS for English content:
      ...
      - Do NOT include any explanation or extra text, only output the JSON object
    `;return await m(E,e)},[l,d,p]=await Promise.all([c("easy"),c("normal"),c("hard")]);return{easy:l,normal:d,hard:p}},qe=async(i,e,t,n,r)=>await we(i,"",e,t,n,r),Ue=async(i,e,t,n,r)=>{const s=/[a-zA-Z]/.test(i),a=`
  ...
  `;return await m(a,n)},M=async(i,e,t,n,r)=>{const s=`
  ...
  `,a=`
    ...
    Do NOT include explanation, only output the JSON array.
  `,[o,c]=await Promise.all([m(s,e),m(a,e)]);return{sentencePractice:o,writingPractice:c,instructions:"這裡提供了造句和寫作練習，幫助學習者提升語言表達能力。造句練習著重於詞彙運用，寫作練習則訓練文章結構和論述能力。完成後可以使用AI批改功能獲得即時回饋。"}},ve=i=>{const e=[];return i.forEach((t,n)=>{if(!t.pairs||!Array.isArray(t.pairs)){e.push(`遊戲 ${n+1}: 缺少 pairs 陣列`);return}const r=new Set,s=new Set;t.pairs.forEach((a,o)=>{if(!a.left||!a.right){e.push(`遊戲 ${n+1}, 配對 ${o+1}: 缺少 left 或 right 內容`);return}r.has(a.left)&&e.push(`遊戲 ${n+1}: 左側內容重複 - "${a.left}"`),s.has(a.right)&&e.push(`遊戲 ${n+1}: 右側內容重複 - "${a.right}"`),r.add(a.left),s.add(a.right)})}),{isValid:e.length===0,issues:e}},Le=async(i,e,t)=>{const n=`
    ...
    Do NOT include any explanation or extra text outside the JSON structure.
  `,r=await m(n,"provider-system-call");if(r.memoryCardGame&&Array.isArray(r.memoryCardGame)){const s=ve(r.memoryCardGame);s.isValid||(console.warn("記憶卡遊戲驗證失敗:",s.issues),r._validationWarnings=s.issues)}return r},x=async(i,e,t,n)=>{const r=`
    Based on the topic "${i}" and learning objectives: ${JSON.stringify(t)}
    ...
    Do NOT include any explanation or extra text. Only output the JSON object.
  `;return await n(r,e)},Ve=async(i,e,t)=>{console.log(`生成學習程度建議 (Provider 系統): ${i}`);const n=await T(i,e);return await x(i,e,n,t)},We=i=>{const e=["english","grammar","vocabulary","pronunciation","speaking","writing","reading","listening","conversation","toefl","ielts","toeic","english literature","business english","academic english","phrasal verbs","idioms","prepositions","tenses","articles","adjectives","adverbs","nouns","verbs","英語","英文","文法","單字","發音","口說","寫作","閱讀","聽力","對話","會話","托福","雅思","多益","商業英文","學術英文","片語動詞","慣用語","介系詞","時態","冠詞","形容詞","副詞","名詞","動詞"],t=i.toLowerCase();return e.some(n=>t.includes(n))},ze=async(i,e,t,n={})=>{console.log(`開始生成學習計劃 (Provider 系統): ${i}`);const r=await T(i,e);return console.log("✓ 學習目標生成完成"),await j(i,r,e,t,n)},j=async(i,e,t,n,r={})=>{console.log(`開始從現有目標生成學習計劃 (Provider 系統): ${i}`);const[s,a,o,c,l,d,p]=await Promise.all([te(i,t,e),ne(i,t,e),re(i,t,e),ie(i,t,e),r.includeEnglishConversation!==!1?se(i,t,e):Promise.resolve([]),x(i,t,e,n),r.includeWritingPractice!==!1?M(i,t,e):Promise.resolve(void 0)]);return console.log("✓ 所有內容生成完成 (Provider 系統)"),{learningObjectives:e,contentBreakdown:s,confusingPoints:a,classroomActivities:o,onlineInteractiveQuiz:c,englishConversation:l,learningLevels:d,writingPractice:p}},Ke=async(i,e,t,n)=>{console.log(`開始生成學習計劃 (有程度設定, Provider 系統): ${i}, 程度: ${e?.name||"未指定"}`);const r=await ae(i,e,t);console.log("✓ 程度特定學習目標生成完成");const[s,a,o,c,l,d,p]=await Promise.all([oe(i,e,t,r),ce(i,e,t,r),le(i,e,t,r),de(i,e,t,r),ue(i,e,t,r),x(i,t,r,n),M(i,t,r,e)]);return console.log("✓ 所有程度特定內容生成完成 (Provider 系統)"),{learningObjectives:r,contentBreakdown:s,confusingPoints:a,classroomActivities:o,onlineInteractiveQuiz:c,englishConversation:l,learningLevels:d,writingPractice:p}},Qe=async(i,e,t,n,r)=>{console.log(`開始生成學習計劃 (有程度和詞彙設定, Provider 系統): ${i}, 程度: ${e?.name||"未指定"}, 詞彙: ${t?.name||"未指定"}`);const s=await pe(i,e,t,n);console.log("✓ 程度和詞彙特定學習目標生成完成");const[a,o,c,l,d,p,g]=await Promise.all([ge(i,e,t,n,s),me(i,e,t,n,s),he(i,e,t,n,s),fe(i,e,t,n,s),ye(i,e,t,n,s),x(i,n,s,r),M(i,n,s,e,t)]);return console.log("✓ 所有程度和詞彙特定內容生成完成 (Provider 系統)"),{learningObjectives:s,contentBreakdown:a,confusingPoints:o,classroomActivities:c,onlineInteractiveQuiz:l,englishConversation:d,learningLevels:p,writingPractice:g}},He=async()=>await O.hasConfiguredProviders(),Ye=async i=>await O.addProvider(i),Xe=async()=>(console.log("Provider 系統初始化中..."),!0),U={preschool:"Preschool",elementary_low:"Elementary School (Grades 1-2)",elementary_mid:"Elementary School (Grades 3-4)",elementary_high:"Elementary School (Grades 5-6)",junior_7:"Junior High School (Grade 7)",junior_8:"Junior High School (Grade 8)",junior_9:"Junior High School (Grade 9)",high_school_review:"High School Review"},L={physical:"Physical Classroom",online:"Online / Remote Learning"},V={none:"Standard / No specific prior issues",partial:"Partial Knowledge / Needs Review",special_needs:"Special Needs / Requires Extra Scaffolding"},Se={visual:"Visual Learning (Diagrams, Graphs)",cpa:"CPA (Concrete-Pictorial-Abstract)",concept_oriented:"Concept-Oriented (Understanding over Rote Memorization)",discovery:"Discovery Learning",gamification:"Gamification",contextual:"Contextual Learning (Real-world applications)"},be={clt:"Communicative Language Teaching (CLT)",tbl:"Task-Based Learning (TBL)",pbl:"Project-Based Learning (PBL)",tpr:"Total Physical Response (TPR)",phonics:"Phonics",grammar_translation:"Grammar-Translation",ppp:"PPP (Presentation, Practice, Production)",cooperative:"Cooperative Learning",gamification:"Gamification",flipped:"Flipped Classroom",scaffolding:"Scaffolding / Educational Drama",clil:"CLIL (Content and Language Integrated Learning)"},Ae=i=>{const{selectedMaterials:e,studentCount:t,classDuration:n,teachingContext:r,priorExperience:s,studentGrade:a,teachingMethod:o}=i;return`
...
- Duration: ${n} minutes
- Setting: ${L[r]||r}
- Student Background: ${V[s]||s}
- Teaching Method: ${Se[o]||o}
`.trim()},$e=i=>{const{selectedMaterials:e,studentCount:t,classDuration:n,teachingContext:r,priorExperience:s,studentGrade:a,teachingMethod:o}=i;return`
...
- Setting: ${L[r]||r}
- Student Background: ${V[s]||s}
- Teaching Method: ${be[o]||o}
`.trim()},Ze=async(i,e)=>{const t=Ae(i);console.log(`開始生成數學學習目標 (Provider 系統): ${t}`);const n=await T(t,e);return{topic:t,learningObjectives:n}},et=async(i,e,t,n)=>(console.log(`開始生成數學學習內容 (Provider 系統): ${i}`),await j(i,e,t,n,{includeEnglishConversation:!1,includeWritingPractice:!1})),tt=async(i,e)=>{const t=$e(i);console.log(`開始生成英語學習目標 (Provider 系統): ${t}`);const n=await T(t,e);return{topic:t,learningObjectives:n}},nt=async(i,e,t,n)=>(console.log(`開始生成英語學習內容 (Provider 系統): ${i}`),await j(i,e,t,n,{includeEnglishConversation:!0,includeWritingPractice:!0})),N=async i=>{try{const e=await K(i);let t=e;if(typeof t=="string"){let n=t.trim();const r=/^```(\w*)?\s*\n?(.*?)\n?\s*```$/s,s=n.match(r);if(s&&s[2]&&(n=s[2].trim()),!n.startsWith("{")&&!n.startsWith("[")&&(n.includes("{")||n.includes("["))){const a=n.indexOf("{"),o=n.indexOf("[");let c=-1,l=-1;a!==-1&&(o===-1||a<o)?(c=a,l=n.lastIndexOf("}")):o!==-1&&(c=o,l=n.lastIndexOf("]")),c!==-1&&l!==-1&&l>c&&(n=n.substring(c,l+1))}try{t=JSON.parse(n)}catch{throw console.error("AI 回傳原始內容 (JSON parse 失敗):",e),new Error("AI 模型傳回的資料格式無法解析 (可能不是有效的 JSON)。請嘗試修改您的主題或重試。")}}return t}catch(e){let t="無法產生診斷內容。";throw e instanceof Error&&(t+=` 詳細資料： ${e.message}`),new Error(t)}},Ee=i=>{const e=new Map;return i.forEach(t=>{e.has(t.questionType)||e.set(t.questionType,{questionType:t.questionType,totalQuestions:0,correctCount:0,accuracy:0,averageTime:0,commonErrors:[],difficultyBreakdown:{easy:{total:0,correct:0,accuracy:0},normal:{total:0,correct:0,accuracy:0},hard:{total:0,correct:0,accuracy:0}}})}),i.forEach(t=>{const n=e.get(t.questionType);n.totalQuestions++,t.isCorrect&&n.correctCount++;const r=n.difficultyBreakdown[t.difficulty];r.total++,t.isCorrect&&r.correct++,r.accuracy=r.total>0?Math.round(r.correct/r.total*100):0}),e.forEach(t=>{t.accuracy=t.totalQuestions>0?Math.round(t.correctCount/t.totalQuestions*100):0;const r=i.filter(s=>s.questionType===t.questionType).filter(s=>s.responseTime).map(s=>s.responseTime);t.averageTime=r.length>0?Math.round(r.reduce((s,a)=>s+a,0)/r.length):void 0}),Array.from(e.values())},Oe=i=>{if(i.length===0)return 0;const e=i.filter(t=>t.isCorrect).length;return Math.round(e/i.length*100)},Ce=i=>i>=80?"advanced":i>=60?"intermediate":"beginner",Pe=async(i,e)=>{const t=e.filter(o=>!o.isCorrect),n=e.filter(o=>o.isCorrect),r=t.map(o=>`
    題型: ${o.questionType}
    難度: ${o.difficulty}
...
`),a=`
  ...
  `;try{return await N(a)}catch(o){return console.error("AI 學習分析生成失敗:",o),{strengths:[],weaknesses:[],learningStyle:"無法分析",cognitivePattern:"需要更多數據"}}},Te=async(i,e,t,n,r)=>{const s=`
  ...
  `;try{return(await N(s)).recommendations||[]}catch(a){return console.error("AI 個人化建議生成失敗:",a),[]}},xe=async(i,e,t,n,r,s,a)=>{const c=(a?.filter(d=>!d.isCorrect)||[]).map(d=>`
  ...
  `;try{const d=await N(l);return{studentId:s,overallScore:e,overallLevel:t,encouragementMessage:d.encouragementMessage||"你的學習表現很棒！",keyStrengths:d.keyStrengths||[],improvementAreas:d.improvementAreas||[],nextSteps:d.nextSteps||[],studyTips:d.studyTips||[],motivationalQuote:d.motivationalQuote||"持續努力，你會更進步！"}}catch(d){return console.error("AI 學生回饋生成失敗:",d),{studentId:s,overallScore:e,overallLevel:t,encouragementMessage:"你的學習表現很棒！繼續保持這個學習態度。",keyStrengths:["積極參與測驗","展現學習動機"],improvementAreas:["需要更多練習"],nextSteps:["重新檢視錯誤題目","加強基礎概念","定期練習"],studyTips:["制定規律的學習時間","多做類似練習題","請教老師或同學"],motivationalQuote:"每一次練習都讓你更接近目標！"}}},Ne=async(i,e,t,n,r,s)=>{const o=(s?.filter(l=>!l.isCorrect)||[]).map(l=>`
  ...
  `;try{const l=await N(c);return{immediateInterventions:l.immediateInterventions||[],instructionalStrategies:l.instructionalStrategies||[],differentiation:l.differentiation||[],parentGuidance:l.parentGuidance||[]}}catch(l){return console.error("AI 教學建議生成失敗:",l),{immediateInterventions:[],instructionalStrategies:[],differentiation:[],parentGuidance:[]}}},rt=async i=>{try{const e=Ee(i.responses),t=Oe(i.responses),n=Ce(t),r=await Pe(i.topic,i.responses),s=await Te(i.topic,t,n,r.strengths,r.weaknesses),a=await xe(i.topic,t,n,r.strengths,r.weaknesses,i.studentId,i.responses),o=await Ne(i.topic,t,e,r.strengths,r.weaknesses,i.responses),c={studentId:i.studentId,assessmentDate:new Date().toISOString(),topic:i.topic,overallPerformance:{totalScore:t,level:n,timeSpent:i.endTime&&i.startTime?Math.round((new Date(i.endTime).getTime()-new Date(i.startTime).getTime())/6e4):0},performanceByType:e,learningAnalysis:r,teachingRecommendations:o,personalizedRecommendations:s,progressTracking:{keyMetrics:["正確率提升","回答速度","概念理解深度"],checkpoints:["一週後複習","兩週後小測驗","一個月後綜合評估"],reassessmentSuggestion:"建議在完成建議的學習活動後，於 2-3 週內進行重新評估"}};return{session:i,studentFeedback:a,teacherReport:c,rawData:{responses:i.responses,statistics:e},generatedAt:new Date().toISOString()}}catch(e){throw console.error("生成學習診斷失敗:",e),new Error("無法生成學習診斷報告，請稍後重試")}},it=(i,e)=>({sessionId:`session_${Date.now()}_${Math.random().toString(36).slice(2,11)}`,studentId:e,topic:i,startTime:new Date().toISOString(),responses:[],isCompleted:!1,metadata:{userAgent:navigator.userAgent,deviceType:/Mobi|Android/i.test(navigator.userAgent)?"mobile":"desktop"}}),st=i=>({...i,endTime:new Date().toISOString(),isCompleted:!0}),C="https://api.jsonbin.io/v3/b",$="$2a$10$0PieG5Wm.V.6svtN4PbaTuVxU4LY0nL2Vaz4N3g2aYlb0cL1NG4Qa";async function at(i){const t=await(await fetch(C,{method:"POST",headers:{"Content-Type":"application/json","X-Master-Key":$,"X-Bin-Private":"false"},body:JSON.stringify(i)})).json();if(!t||!t.metadata||!t.metadata.id)throw new Error("無法取得 jsonbin id");return t.metadata.id}async function ot(i){const t=await(await fetch(`https://api.jsonbin.io/v3/b/${i}/latest`,{})).json();if(!t||!t.record)throw new Error("找不到對應的教學方案");return t.record}async function ct(i){const e={type:"quiz",quiz:i.quiz,topic:i.topic,apiKey:i.apiKey,metadata:i.metadata,createdAt:new Date().toISOString(),supportsDiagnostic:!!i.apiKey},n=await(await fetch(C,{method:"POST",headers:{"Content-Type":"application/json","X-Master-Key":$,"X-Bin-Private":"false"},body:JSON.stringify(e)})).json();if(!n||!n.metadata||!n.metadata.id)throw new Error("無法儲存測驗內容");return n.metadata.id}async function lt(i){const t=await(await fetch(`https://api.jsonbin.io/v3/b/${i}/latest`,{})).json();if(!t||!t.record)throw new Error("找不到對應的測驗內容");const n=t.record;if(n.type!=="quiz")throw new Error("此連結不是測驗專用分享");return{quiz:n.quiz,topic:n.topic,apiKey:n.apiKey,supportsDiagnostic:n.supportsDiagnostic||!1,metadata:n.metadata}}async function dt(i){const e={type:"writing",writingPractice:i.writingPractice,topic:i.topic,metadata:i.metadata,createdAt:new Date().toISOString()},n=await(await fetch(C,{method:"POST",headers:{"Content-Type":"application/json","X-Master-Key":$,"X-Bin-Private":"false"},body:JSON.stringify(e)})).json();if(!n||!n.metadata||!n.metadata.id)throw new Error("無法儲存寫作練習內容");return n.metadata.id}async function ut(i){const t=await(await fetch(`https://api.jsonbin.io/v3/b/${i}/latest`,{})).json();if(!t||!t.record)throw new Error("找不到對應的寫作練習內容");const n=t.record;if(n.type!=="writing")throw new Error("此連結不是寫作練習專用分享");return{writingPractice:n.writingPractice,topic:n.topic,metadata:n.metadata}}async function pt(i){const e={type:"student-results",...i,createdAt:new Date().toISOString()},n=await(await fetch(C,{method:"POST",headers:{"Content-Type":"application/json","X-Master-Key":$,"X-Bin-Private":"false"},body:JSON.stringify(e)})).json();if(!n||!n.metadata||!n.metadata.id)throw new Error("無法儲存學生作答結果");return n.metadata.id}async function Ie(i){const t=await(await fetch(`https://api.jsonbin.io/v3/b/${i}/latest`,{})).json();if(!t||!t.record)throw new Error("找不到對應的學生作答結果");const n=t.record;if(n.type!=="student-results")throw new Error("此連結不是學生作答結果專用分享");return{studentName:n.studentName,topic:n.topic,difficulty:n.difficulty,responses:n.responses,overallScore:n.overallScore,completedAt:n.completedAt,quizBinId:n.quizBinId,quizContent:n.quizContent,metadata:n.metadata,diagnosticReport:n.diagnosticReport}}async function gt(i,e){const r={type:"student-results",...{...await Ie(i),diagnosticReport:{...e,generatedAt:new Date().toISOString(),resultsBinId:i},updatedAt:new Date().toISOString()}},a=await(await fetch(`https://api.jsonbin.io/v3/b/${i}`,{method:"PUT",headers:{"Content-Type":"application/json","X-Master-Key":$},body:JSON.stringify(r)})).json();if(!a||!a.metadata)throw new Error("無法更新學生作答結果")}async function mt(i){const e={type:"conversation-practice",practice:i.practice,metadata:i.metadata,createdAt:new Date().toISOString()},n=await(await fetch(C,{method:"POST",headers:{"Content-Type":"application/json","X-Master-Key":$,"X-Bin-Private":"false"},body:JSON.stringify(e)})).json();if(!n||!n.metadata||!n.metadata.id)throw new Error("無法儲存英文對話練習內容");return n.metadata.id}async function ht(i){const t=await(await fetch(`https://api.jsonbin.io/v3/b/${i}/latest`,{})).json();if(!t||!t.record)throw new Error("找不到對應的分享內容");return t.record}class ke{async generateConversationPractice(e){const{callGemini:t}=await R(async()=>{const{callGemini:a}=await import("./gemini-service-Byb1d6yD.js").then(o=>o.q);return{callGemini:a}},__vite__mapDeps([0,1,2,3])),n=this.buildConversationPrompt(e),r=await t(n);return{id:`conv_${Date.now()}_${Math.random().toString(36).substr(2,9)}`,...r}}buildConversationPrompt(e){return`
...
`}async generateConversationFeedback(e,t,n){const{callGemini:r}=await R(async()=>{const{callGemini:a}=await import("./gemini-service-Byb1d6yD.js").then(o=>o.q);return{callGemini:a}},__vite__mapDeps([0,1,2,3])),s=`
...
`;return await r(s)}getConversationTemplates(){return[{category:"日常生活 (Daily Life)",scenarios:[{title:"在咖啡廳點餐",scenario:"Student plays customer ordering coffee and snacks",suggestedDifficulty:"easy",participants:["顧客","服務員"]},{title:"問路和指路",scenario:"Tourist asking for directions to nearby attractions",suggestedDifficulty:"easy",participants:["遊客","當地人"]},{title:"在超市購物",scenario:"Shopping for groceries and asking about products",suggestedDifficulty:"normal",participants:["顧客","店員"]}]},{category:"商務英文 (Business English)",scenarios:[{title:"工作面試",scenario:"Job interview for entry-level position",suggestedDifficulty:"hard",participants:["求職者","面試官"]},{title:"會議討論",scenario:"Team meeting discussing project progress",suggestedDifficulty:"hard",participants:["經理","員工A","員工B"]},{title:"客戶服務",scenario:"Handling customer complaints and inquiries",suggestedDifficulty:"normal",participants:["客服代表","顧客"]}]},{category:"旅遊英文 (Travel English)",scenarios:[{title:"機場報到",scenario:"Checking in at the airport and asking about flights",suggestedDifficulty:"normal",participants:["旅客","地勤人員"]},{title:"飯店住宿",scenario:"Checking into hotel and requesting services",suggestedDifficulty:"easy",participants:["房客","櫃台人員"]},{title:"觀光景點",scenario:"Visiting tourist attractions and asking about information",suggestedDifficulty:"normal",participants:["遊客","導遊"]}]},{category:"學術英文 (Academic English)",scenarios:[{title:"課堂討論",scenario:"Participating in classroom discussion about a topic",suggestedDifficulty:"hard",participants:["學生","教授"]},{title:"圖書館詢問",scenario:"Asking librarian for help finding resources",suggestedDifficulty:"normal",participants:["學生","圖書館員"]}]}]}getDifficultyLevels(){return[{level:"easy",description:"初級 - 適合英文基礎學習者",characteristics:["使用基本詞彙和簡單句型","現在式和過去式為主","對話內容直接明確","較短的對話輪次","日常生活場景"]},{level:"normal",description:"中級 - 適合有一定英文基礎的學習者",characteristics:["混合使用多種時態","中等難度詞彙","複合句和複雜句型","需要推理和延伸回應","工作和社交場景"]},{level:"hard",description:"高級 - 適合英文程度較好的學習者",characteristics:["高級詞彙和慣用語","複雜的語法結構","需要批判性思考","抽象概念討論","專業和學術場景"]}]}}const ft=new ke;class Re{constructor(){this.dbName="AILearningPageGenerator",this.version=1,this.storeName="lessonPlans",this.db=null}async init(){return new Promise((e,t)=>{const n=indexedDB.open(this.dbName,this.version);n.onerror=()=>{t(new Error("Failed to open IndexedDB"))},n.onsuccess=r=>{this.db=r.target.result,e()},n.onupgradeneeded=r=>{const s=r.target.result;if(!s.objectStoreNames.contains(this.storeName)){const a=s.createObjectStore(this.storeName,{keyPath:"id"});a.createIndex("topic","topic",{unique:!1}),a.createIndex("createdAt","createdAt",{unique:!1}),a.createIndex("lastAccessedAt","lastAccessedAt",{unique:!1})}}})}async ensureDB(){if(this.db||await this.init(),!this.db)throw new Error("Database not initialized");return this.db}async saveLessonPlan(e){const t=await this.ensureDB();return new Promise((n,r)=>{const o=t.transaction([this.storeName],"readwrite").objectStore(this.storeName).put(e);o.onsuccess=()=>n(),o.onerror=()=>r(new Error("Failed to save lesson plan"))})}async getAllLessonPlans(){const e=await this.ensureDB();return new Promise((t,n)=>{const o=e.transaction([this.storeName],"readonly").objectStore(this.storeName).index("lastAccessedAt").openCursor(null,"prev"),c=[];o.onsuccess=l=>{const d=l.target.result;d?(c.push(d.value),d.continue()):t(c)},o.onerror=()=>n(new Error("Failed to get lesson plans"))})}async getLessonPlan(e){const t=await this.ensureDB();return new Promise((n,r)=>{const o=t.transaction([this.storeName],"readonly").objectStore(this.storeName).get(e);o.onsuccess=()=>{const c=o.result;n(c||null)},o.onerror=()=>r(new Error("Failed to get lesson plan"))})}async updateLastAccessed(e){const t=await this.getLessonPlan(e);t&&(t.lastAccessedAt=new Date().toISOString(),await this.saveLessonPlan(t))}async deleteLessonPlan(e){const t=await this.ensureDB();return new Promise((n,r)=>{const o=t.transaction([this.storeName],"readwrite").objectStore(this.storeName).delete(e);o.onsuccess=()=>n(),o.onerror=()=>r(new Error("Failed to delete lesson plan"))})}async searchLessonPlans(e){const t=await this.getAllLessonPlans(),n=e.toLowerCase();return t.filter(r=>r.topic.toLowerCase().includes(n))}async getSharedLessonPlans(){return(await this.getAllLessonPlans()).filter(t=>t.metadata?.isShared===!0)}async getLocalLessonPlans(){return(await this.getAllLessonPlans()).filter(t=>!t.metadata?.isShared)}async cleanupOldLessonPlans(e=50){const t=await this.getAllLessonPlans();if(t.length>e){const n=t.slice(e);for(const r of n)await this.deleteLessonPlan(r.id)}}async getStorageStats(){const e=await this.getAllLessonPlans();return{totalCount:e.length,totalSizeEstimate:JSON.stringify(e).length,oldestPlan:e[e.length-1]?.topic,newestPlan:e[0]?.topic}}}const yt=new Re,Me=()=>{const i=Date.now(),e=Math.random().toString(36).substr(2,9);return`lesson_${i}_${e}`},wt=(i,e)=>{const t=new Date().toISOString();return{id:Me(),topic:i,createdAt:t,lastAccessedAt:t,content:e,metadata:{totalSections:[e.learningObjectives,e.contentBreakdown,e.confusingPoints,e.classroomActivities,e.quiz,e.writingPractice].filter(Boolean).length,hasQuiz:!!e.quiz,hasWriting:!!e.writingPractice}}};class je{constructor(){this.dbName="InteractiveContentDB",this.version=1,this.db=null}async init(){if(!this.db)return new Promise((e,t)=>{const n=indexedDB.open(this.dbName,this.version);n.onerror=()=>{t(new Error("Failed to open InteractiveContent database"))},n.onsuccess=()=>{this.db=n.result,e()},n.onupgradeneeded=r=>{const s=r.target.result;if(!s.objectStoreNames.contains("interactiveContent")){const a=s.createObjectStore("interactiveContent",{keyPath:"lessonPlanId"});a.createIndex("topic","topic",{unique:!1}),a.createIndex("lastAccessedAt","lastAccessedAt",{unique:!1})}}})}ensureDB(){if(!this.db)throw new Error("Database not initialized. Call init() first.");return this.db}async saveVersion(e,t,n,r,s,a){console.log("=== interactiveContentStorage.saveVersion 開始 ==="),console.log("參數:",{lessonPlanId:e,topic:t,transformedData:n,selectedSteps:r,versionName:s,description:a});try{const o=this.ensureDB();console.log("✅ 獲取資料庫連接成功");const c=`version_${Date.now()}`,l=new Date().toISOString();return console.log("生成版本ID:",c),new Promise((d,p)=>{const g=o.transaction(["interactiveContent"],"readwrite"),u=g.objectStore("interactiveContent");console.log("✅ 開始資料庫事務");const f=u.get(e);console.log("查詢現有記錄:",e),f.onsuccess=()=>{console.log("✅ 查詢現有記錄成功");const h=f.result;console.log("現有記錄:",h?`存在，包含 ${h.versions.length} 個版本`:"不存在");const E={id:c,name:s||`版本 ${l.slice(0,19).replace("T"," ")}`,description:a,createdAt:l,lastModifiedAt:l,transformedData:n,selectedSteps:r};console.log("新版本物件:",E);const _={lessonPlanId:e,topic:t,versions:h?[...h.versions,E]:[E],lastAccessedAt:l};console.log("準備保存記錄:",_);const P=u.put(_);P.onsuccess=()=>{console.log("✅ 版本保存成功"),d(c)},P.onerror=W=>{console.error("❌ 保存版本失敗:",W),console.error("putRequest.error:",P.error),p(new Error(`Failed to save interactive content version: ${P.error?.message||"Unknown error"}`))}},f.onerror=h=>{console.error("❌ 查詢現有記錄失敗:",h),console.error("getRequest.error:",f.error),p(new Error(`Failed to get existing interactive content: ${f.error?.message||"Unknown error"}`))},g.onerror=h=>{console.error("❌ 資料庫事務失敗:",h),p(new Error(`Database transaction failed: ${g.error?.message||"Unknown error"}`))}})}catch(o){throw console.error("❌ interactiveContentStorage.saveVersion 異常:",o),o}}async getVersions(e){const t=this.ensureDB();return new Promise((n,r)=>{const o=t.transaction(["interactiveContent"],"readonly").objectStore("interactiveContent").get(e);o.onsuccess=()=>{const c=o.result;n(c?c.versions:[])},o.onerror=()=>{r(new Error("Failed to get interactive content versions"))}})}async getVersion(e,t){return(await this.getVersions(e)).find(r=>r.id===t)||null}async updateVersion(e,t,n){const r=this.ensureDB();return new Promise((s,a)=>{const c=r.transaction(["interactiveContent"],"readwrite").objectStore("interactiveContent"),l=c.get(e);l.onsuccess=()=>{const d=l.result;if(!d){a(new Error("Interactive content record not found"));return}const p=d.versions.findIndex(u=>u.id===t);if(p===-1){a(new Error("Version not found"));return}d.versions[p]={...d.versions[p],...n,lastModifiedAt:new Date().toISOString()};const g=c.put(d);g.onsuccess=()=>s(),g.onerror=()=>a(new Error("Failed to update version"))},l.onerror=()=>{a(new Error("Failed to get interactive content"))}})}async deleteVersion(e,t){const n=this.ensureDB();return new Promise((r,s)=>{const o=n.transaction(["interactiveContent"],"readwrite").objectStore("interactiveContent"),c=o.get(e);c.onsuccess=()=>{const l=c.result;if(!l){s(new Error("Interactive content record not found"));return}if(l.versions=l.versions.filter(d=>d.id!==t),l.versions.length===0){const d=o.delete(e);d.onsuccess=()=>r(),d.onerror=()=>s(new Error("Failed to delete record"))}else{const d=o.put(l);d.onsuccess=()=>r(),d.onerror=()=>s(new Error("Failed to update record"))}},c.onerror=()=>{s(new Error("Failed to get interactive content"))}})}async getAllRecords(){const e=this.ensureDB();return new Promise((t,n)=>{const a=e.transaction(["interactiveContent"],"readonly").objectStore("interactiveContent").getAll();a.onsuccess=()=>{t(a.result)},a.onerror=()=>{n(new Error("Failed to get all interactive content records"))}})}async updateLastAccessed(e){const t=this.ensureDB();return new Promise((n,r)=>{const a=t.transaction(["interactiveContent"],"readwrite").objectStore("interactiveContent"),o=a.get(e);o.onsuccess=()=>{const c=o.result;if(c){c.lastAccessedAt=new Date().toISOString();const l=a.put(c);l.onsuccess=()=>n(),l.onerror=()=>r(new Error("Failed to update last accessed time"))}else n()},o.onerror=()=>{r(new Error("Failed to get interactive content"))}})}}const vt=new je;class _e{constructor(){this.synthesis=null,this.recognition=null,this.isRecording=!1,this.initializeSpeechSynthesis(),this.initializeSpeechRecognition()}initializeSpeechSynthesis(){"speechSynthesis"in window&&(this.synthesis=window.speechSynthesis)}isTTSSupported(){return this.synthesis!==null}getAvailableVoices(){return this.synthesis?this.synthesis.getVoices().filter(e=>e.lang.startsWith("en")):[]}speak(e,t={},n,r,s){return new Promise((a,o)=>{if(!this.synthesis){o(new Error("Text-to-Speech not supported in this browser"));return}this.synthesis.cancel();const c=new SpeechSynthesisUtterance(e),d={...{lang:"en-US",rate:1,pitch:1,volume:1},...t};c.lang=d.lang,c.rate=d.rate,c.pitch=d.pitch,c.volume=d.volume;const p=this.getAvailableVoices(),g=p.find(u=>u.lang===d.lang&&!u.name.includes("Google"))||p.find(u=>u.lang===d.lang)||p[0];g&&(c.voice=g),c.onstart=()=>{n?.()},c.onend=()=>{r?.(),a()},c.onerror=u=>{s?.(u),o(u)},this.synthesis.speak(c)})}stopSpeaking(){this.synthesis&&this.synthesis.cancel()}pauseSpeaking(){this.synthesis&&this.synthesis.pause()}resumeSpeaking(){this.synthesis&&this.synthesis.resume()}isSpeaking(){return this.synthesis?.speaking||!1}initializeSpeechRecognition(){const e=window.SpeechRecognition||window.webkitSpeechRecognition||window.mozSpeechRecognition||window.msSpeechRecognition;e&&(this.recognition=new e)}isSTTSupported(){return this.recognition!==null}startRecording(e={},t,n,r,s){return new Promise((a,o)=>{if(!this.recognition){o(new Error("Speech recognition not supported in this browser"));return}if(this.isRecording){o(new Error("Already recording"));return}const l={...{lang:"en-US",continuous:!1,interimResults:!0,maxAlternatives:3},...e};this.recognition.lang=l.lang,this.recognition.continuous=l.continuous,this.recognition.interimResults=l.interimResults,this.recognition.maxAlternatives=l.maxAlternatives,this.recognition.onstart=()=>{this.isRecording=!0,r?.()},this.recognition.onresult=d=>{for(let p=d.resultIndex;p<d.results.length;p++){const g=d.results[p][0].transcript,u=d.results[p][0].confidence;t?.({text:g,confidence:u||.8,isFinal:d.results[p].isFinal})}},this.recognition.onerror=d=>{this.isRecording=!1,n?.(d),o(d)},this.recognition.onend=()=>{this.isRecording=!1,s?.(),a()};try{this.recognition.start()}catch(d){this.isRecording=!1,o(d)}})}stopRecording(){this.recognition&&this.isRecording&&this.recognition.stop()}isCurrentlyRecording(){return this.isRecording}analyzePronunciation(e,t){const n=e.toLowerCase().split(/\s+/),r=t.toLowerCase().split(/\s+/),s=[],a=[];n.forEach(l=>{const d=l.replace(/[.,!?;:]/g,"");r.some(p=>p.replace(/[.,!?;:]/g,"").includes(d)||d.includes(p.replace(/[.,!?;:]/g,"")))?s.push(l):a.push(l)});const o=n.length>0?s.length/n.length*100:0,c=[];return o<70&&c.push("Try speaking more slowly and clearly"),a.length>0&&c.push(`Practice these words: ${a.slice(0,3).join(", ")}`),t.length<e.length*.5&&c.push("Speak louder and closer to the microphone"),{accuracy:Math.round(o),suggestions:c,matchedWords:s,missedWords:a}}getSupportStatus(){const e=navigator.userAgent;let t="Unknown browser";return e.includes("Chrome")?t="Chrome":e.includes("Firefox")?t="Firefox":e.includes("Safari")?t="Safari":e.includes("Edge")&&(t="Edge"),{ttsSupported:this.isTTSSupported(),sttSupported:this.isSTTSupported(),browserInfo:t}}}const St=new _e;class F{static async encrypt(e,t){try{const n=crypto.getRandomValues(new Uint8Array(16)),r=await crypto.subtle.importKey("raw",new TextEncoder().encode(t),{name:"PBKDF2"},!1,["deriveKey"]),s=await crypto.subtle.deriveKey({name:"PBKDF2",salt:n,iterations:1e5,hash:"SHA-256"},r,{name:"AES-GCM",length:256},!1,["encrypt"]),a=crypto.getRandomValues(new Uint8Array(12)),o=await crypto.subtle.encrypt({name:"AES-GCM",iv:a},s,new TextEncoder().encode(e)),c=new Uint8Array(n.length+a.length+o.byteLength);return c.set(n,0),c.set(a,n.length),c.set(new Uint8Array(o),n.length+a.length),btoa(String.fromCharCode(...c))}catch(n){throw console.error("加密失敗:",n),new Error("加密數據失敗")}}static async decrypt(e,t){try{const n=new Uint8Array(atob(e).split("").map(d=>d.charCodeAt(0))),r=n.slice(0,16),s=n.slice(16,28),a=n.slice(28),o=await crypto.subtle.importKey("raw",new TextEncoder().encode(t),{name:"PBKDF2"},!1,["deriveKey"]),c=await crypto.subtle.deriveKey({name:"PBKDF2",salt:r,iterations:1e5,hash:"SHA-256"},o,{name:"AES-GCM",length:256},!1,["decrypt"]),l=await crypto.subtle.decrypt({name:"AES-GCM",iv:s},c,a);return new TextDecoder().decode(l)}catch(n){throw console.error("解密失敗:",n),new Error("解密數據失敗 - 請檢查密碼是否正確")}}static generateRandomPassword(){const e="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!@#$%^&*";let t="";for(let n=0;n<16;n++)t+=e.charAt(Math.floor(Math.random()*e.length));return t}static validatePasswordStrength(e){const t=[];let n=0;return e.length<8?t.push("密碼長度至少需要 8 個字符"):n+=1,/[a-z]/.test(e)?n+=1:t.push("需要包含小寫字母"),/[A-Z]/.test(e)?n+=1:t.push("需要包含大寫字母"),/[0-9]/.test(e)?n+=1:t.push("需要包含數字"),/[!@#$%^&*()_+\-=[\]{};':"\\|,.<>/?]/.test(e)?n+=1:t.push("建議包含特殊字符"),{isValid:n>=3,score:n,feedback:t}}}const I="https://api.jsonbin.io/v3",k="$2a$10$0PieG5Wm.V.6svtN4PbaTuVxU4LY0nL2Vaz4N3g2aYlb0cL1NG4Qa";class bt{static async createEncryptedShare(e,t,n,r){try{const s={providers:e.map(d=>({type:d.type,name:d.name,model:this.getProviderModel(d),apiKey:d.apiKey,settings:this.getProviderSettings(d),description:d.description})),sharedAt:new Date().toISOString()},a=await F.encrypt(JSON.stringify(s),n),o={id:`encrypted_${Date.now()}_${Math.random().toString(36).substr(2,9)}`,name:t,description:r,encryptedData:a,metadata:{version:"1.0",createdAt:new Date().toISOString(),providerTypes:[...new Set(e.map(d=>d.type))],providerCount:e.length,hasEncryption:!0}},c=await fetch(`${I}/b`,{method:"POST",headers:{"Content-Type":"application/json","X-Master-Key":k},body:JSON.stringify(o)});if(!c.ok){const d=await c.text();throw new Error(`上傳失敗: ${c.statusText} - ${d}`)}return(await c.json()).metadata.id}catch(s){throw console.error("創建加密分享失敗:",s),s}}static createLegacyShare(e){return`${window.location.origin}/ai-page-gen/?apikey=${encodeURIComponent(e)}`}static async loadEncryptedShare(e,t){try{const n=await fetch(`${I}/b/${e}/latest`,{headers:{"X-Master-Key":k}});if(!n.ok){const o=await n.text();throw new Error(`載入失敗: ${n.statusText} - ${o}`)}const s=(await n.json()).record;if(!s.metadata?.hasEncryption)throw new Error("這不是一個加密的分享配置");const a=await F.decrypt(s.encryptedData,t);return JSON.parse(a)}catch(n){throw console.error("載入加密分享失敗:",n),n}}static async getSharePreview(e){try{const t=await fetch(`${I}/b/${e}/latest`,{headers:{"X-Master-Key":k}});if(!t.ok){const s=await t.text();throw new Error(`載入預覽失敗: ${t.statusText} - ${s}`)}const r=(await t.json()).record;return{name:r.name,description:r.description,providerTypes:r.metadata.providerTypes,providerCount:r.metadata.providerCount,createdAt:r.metadata.createdAt,hasEncryption:r.metadata.hasEncryption}}catch(t){throw console.error("獲取分享預覽失敗:",t),t}}static generateShareUrl(e){return`${window.location.origin}/ai-page-gen/?provider_share=${encodeURIComponent(e)}`}static parseShareUrl(){const e=new URLSearchParams(window.location.search),t=e.get("provider_share");if(t)return{type:"provider",value:t};const n=e.get("apikey");return n?{type:"legacy",value:n}:{type:null,value:null}}static getProviderModel(e){return e.type==="gemini"?e.model||"gemini-2.5-flash":e.type==="openrouter"?e.model||"openai/gpt-4o":"unknown"}static getProviderSettings(e){return e.type==="gemini"?e.settings||{}:e.type==="openrouter"?e.settings||{}:{}}static validateShareData(e){const t=[];return!e.providers||!Array.isArray(e.providers)?t.push("無效的 providers 數據"):e.providers.forEach((n,r)=>{n.type||t.push(`Provider ${r+1}: 缺少類型`),n.name||t.push(`Provider ${r+1}: 缺少名稱`),n.apiKey||t.push(`Provider ${r+1}: 缺少 API Key`),n.model||t.push(`Provider ${r+1}: 缺少模型`)}),e.sharedAt||t.push("缺少分享時間"),{isValid:t.length===0,errors:t}}static createProviderConfigsFromShare(e,t="分享的"){return e.providers.map((n,r)=>{const s={id:`shared_${Date.now()}_${r}`,name:`${t} ${n.name}`,type:n.type,enabled:!0,apiKey:n.apiKey,description:n.description||`從分享匯入的 ${n.type} 配置`,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};return n.type==="gemini"?{...s,model:n.model,settings:n.settings||{responseMimeType:"application/json",temperature:.7,maxOutputTokens:8192}}:n.type==="openrouter"?{...s,model:n.model,settings:n.settings||{temperature:.7,max_tokens:8192}}:s})}static generateShareStats(e){const t={};return e.forEach(n=>{t[n.type]=(t[n.type]||0)+1}),{totalProviders:e.length,providerTypes:t,estimatedCost:"免費",securityLevel:"high"}}}export{O as $,de as A,ue as B,pe as C,ge as D,me as E,he as F,fe as G,ye as H,M as I,qe as J,Le as K,Ue as L,ot as M,yt as N,at as O,vt as P,rt as Q,gt as R,it as S,st as T,ft as U,mt as V,dt as W,ct as X,w as Y,Y as Z,X as _,Qe as a,b as a0,y as a1,S as a2,Oe as a3,pt as a4,lt as a5,ut as a6,Ie as a7,St as a8,ht as a9,F as aa,bt as ab,wt as ac,Ke as b,ze as c,Xe as d,m as e,Je as f,Ve as g,He as h,We as i,Ge as j,et as k,Ze as l,nt as m,tt as n,Ye as o,T as p,te as q,ne as r,re as s,Fe as t,ie as u,se as v,ae as w,oe as x,ce as y,le as z};
