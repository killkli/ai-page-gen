...
const ProviderSettingsModal: React.FC<ProviderSettingsModalProps> = ({
  ...
  const [providers, setProviders] = useState<ProviderConfig[]>(currentProviders);
  ...
  const handleSetDefaultProvider = async (providerId: string) => {
    try {
      ...
      alert(`設置預設 Provider 失敗: ${error instanceof Error ? error.message : '未知錯誤'}`);
    }
  };
  ...
  const handleSaveProvider = async () => {
    ...
    try {
      ...
      const existingIndex = providers.findIndex(p => p.id === selectedProvider.id);
  ...
  const renderTestStatus = (providerId: string) => {
    ...
    return (
      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${statusColors[result.status]}`}>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg max-w-6xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-900">AI Provider 設定</h2>
          <p className="text-sm text-gray-600 mt-1">管理您的 AI Provider 配置和 API 金鑰</p>
        </div>
          ...
          <div className="w-1/2 border-r border-gray-200 flex flex-col">
            <div className="px-6 py-4 border-b border-gray-200">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-medium text-gray-900">Provider 清單</h3>
              ...
              <div className="space-y-4">
                {providers.map((provider) => (
                  <div
                    ...
                    className={`p-4 border rounded-lg cursor-pointer transition-colors ${
                      selectedProvider?.id === provider.id ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200 hover:border-gray-300'
                    }`}
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {provider.enabled && defaultProviderId !== provider.id && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                        ...
                        <button
                          ...
                          title={isDeleting === provider.id ? "刪除中..." : "刪除"}
                        >
                ...
                {providers.length === 0 && (
                  <div className="text-center py-8 text-gray-500">
                    <p>尚未設定任何 Provider</p>
                    <p className="text-sm mt-1">點擊上方按鈕添加新的 Provider</p>
                  </div>
                )}
              </div>
            </div>
            ...
            {isEditing && selectedProvider ? (
              ...
              <div className="flex-1 flex items-center justify-center text-gray-500">
                <div className="text-center">
                  ...
                  <p className="text-sm mt-1">或點擊上方按鈕添加新的 Provider</p>
                </div>
              </div>
            )}
          </div>
    ...
    <div className="flex flex-col h-full">
      <div className="px-6 py-4 border-b border-gray-200">
        <h3 className="text-lg font-medium text-gray-900">
          {provider.id.includes('_') && provider.id.split('_')[1] === Date.now().toString() ? '新增' : '編輯'} Provider
        </h3>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => updateProvider({ description: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  rows={3}
              ...
              <div className="flex items-center">
                ...
                />
                <label className="ml-2 block text-sm text-gray-900">啟用此 Provider</label>
              </div>
            </div>
          </div>
          ...
          {provider.type === ProviderType.GEMINI && (
            <div>
              <h4 className="text-sm font-medium text-gray-900 mb-3">Gemini 設定</h4>
              <div className="space-y-4">
                <ModelSelector
                  provider={provider}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">溫度 (Temperature)</label>
                  <input
                    type="number"
                    min="0"
                    max="2"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">最大輸出 Tokens</label>
                  <input
                    type="number"
                    min="1"
                    max="32768"
                    value={(provider.settings as GeminiProviderConfig['settings']).maxOutputTokens || 8192}
          ...
          {provider.type === ProviderType.OPENROUTER && (
            <div>
              <h4 className="text-sm font-medium text-gray-900 mb-3">OpenRouter 設定</h4>
              <div className="space-y-4">
                <ModelSelector
                  provider={provider}
                ...
                <div>
                  ...
                  <input
                    ...
                    min="1"
                    max="200000"
                    value={(provider.settings as OpenRouterProviderConfig['settings']).max_tokens || 4096}
                ...
                <div className="flex items-center">
                  <input
                    type="checkbox"
                    checked={(provider.settings as OpenRouterProviderConfig['settings']).stream || false}
                    onChange={(e) => updateSettings({ stream: e.target.checked })}
                    className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
                  />
                  <label className="ml-2 block text-sm text-gray-900">啟用串流回應</label>
                </div>
              </div>
            </div>
          )}
