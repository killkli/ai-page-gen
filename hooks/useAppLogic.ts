...
export const useAppLogic = () => {
  ...
  const [generatedContent, setGeneratedContent] =
    useState<ExtendedLearningContent | null>(null);
  ...
  // 新的分享功能狀態
  const [showProviderShareModal, setShowProviderShareModal] =
    useState<boolean>(false);
  const [availableProviders, setAvailableProviders] = useState<any[]>([]);

  // 新增：程度建議相關狀態
  const [learningLevels, setLearningLevels] =
    useState<LearningLevelSuggestions | null>(null);
  const [selectedLevel, setSelectedLevel] = useState<LearningLevel | null>(
    null
  );
  const [showingLevelSelection, setShowingLevelSelection] =
    useState<boolean>(false);

  // 新增：單字程度相關狀態 (僅適用於英語主題)
  const [selectedVocabularyLevel, setSelectedVocabularyLevel] =
    useState<VocabularyLevel>(VOCABULARY_LEVELS[0]);
  const [showingVocabularySelection, setShowingVocabularySelection] =
    useState<boolean>(false);
  ...
  // 新增：通用生成器顯示狀態
  const [showGeneralGenerator, setShowGeneralGenerator] =
    useState<boolean>(false);
  ...
  const handleGenerateLevelSuggestions = useCallback(async () => {
    ...
    try {
      ...
      const effectiveApiKey = hasProviders
        ? 'provider-system-placeholder-key'
        : '';
      const levels = await generateLearningLevelSuggestions(
        topic,
        effectiveApiKey
      );
  ...
  // 第二階段：根據選定程度產生完整內容
  const handleGenerateContentWithLevel = useCallback(
    async (level: LearningLevel) => {
      // 檢查是否有配置的 Provider 或舊版 API Key
      const hasProviders = await providerService.hasConfiguredProviders();
      if (!hasProviders && !apiKey) {
        setError('請先配置 AI Provider 或設定 API 金鑰。');
      ...
      try {
        ...
        // 根據是否為英語主題選擇不同的生成方式
        if (isEnglishTopic && selectedVocabularyLevel) {
          content = await generateLearningPlanWithVocabularyLevel(
            topic,
            level,
          ...
          content = await generateLearningPlanWithLevel(
            topic,
            level,
            effectiveApiKey
          );
  ...
  const handleMathComplete = useCallback(
    async (content: ExtendedLearningContent, topicSummary: string) => {
      setGeneratedContent(content);
  ...
  // 英語生成完成處理
  const handleEnglishComplete = useCallback(
    async (content: ExtendedLearningContent, topicSummary: string) => {
      setGeneratedContent(content);
      setTopic(topicSummary);
